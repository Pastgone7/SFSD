...
from modules.FAM import FAM

__all__ = ['ResNet', 'de_resnet18', 'de_resnet34', 'de_resnet50', 'de_resnet101',
...
class Bottleneck_sjz(nn.Module):
    ...
    def __init__(
        ...
        self.upsample = upsample
        self.stride = stride
        self.fam = FAM(planes * self.expansion, stage)
...
class ResNet(nn.Module):
    def __init__(
        ...
        # 根据wideresnet50 bottleneck输出，写死为2028
        ...
        self.layer1 = self._make_layer(Bottleneck_sjz, 1, 256, layers[0], stride=2)
        self.layer2 = self._make_layer(Bottleneck_sjz, 2, 128, layers[1], stride=2,
                                       dilate=replace_stride_with_dilation[0])
        self.layer3 = self._make_layer(Bottleneck_g, 3, 64, layers[2], stride=2,
    ...
    def _make_layer(self, block, stage, planes: int, blocks: int,
        ...
        if stride != 1 or self.inplanes != planes * block.expansion:
            upsample = nn.Sequential(
                ...
                EUCB(in_channels=self.inplanes, out_channels=planes * block.expansion),
                norm_layer(planes * block.expansion),
            )

        layers = []
        layers.append(block(self.inplanes, planes, stride, upsample, self.groups,
                            self.base_width, previous_dilation, norm_layer, stage=stage))
        self.inplanes = planes * block.expansion
        for i in range(1, blocks):
            if i != blocks - stage:
                layers.append(block(self.inplanes, planes, groups=self.groups,
                                    base_width=self.base_width, dilation=self.dilation,
                                    norm_layer=norm_layer, stage=stage))
            else:
                layers.append(Bottleneck_g(self.inplanes, planes, groups=self.groups,
                                base_width=self.base_width, dilation=self.dilation,
    ...
    def _forward_impl(self, x) -> Tensor:
        ...
        feature_a = self.layer1(x[0])  # 2048*8*8->1024*16*16
        # x = self.alpha * feature_a + (1 - self.alpha) * x[1]
        feature_b = self.layer2(x[1])  # 1024*16*16->512*32*32
        feature_c = self.layer3(feature_b)  # 512*32*32->256*64*64
        # 输出前加了relu函数，tensor值有改变，但是异常图的值一模一样
...
if __name__ == '__main__':
    tensor1 = torch.randn(1, 2048, 8, 8)
    tensor2 = torch.randn(1, 1024, 16, 16)
    tensor3 = torch.randn(1, 2048, 16, 16)
    # conv = conv1x1(in_planes=2048, out_planes=1024, stride=1)
    decoder = de_wide_resnet50_2(pretrained=False)
    with torch.no_grad():
        # outputs = conv(tensor2)
        # print(outputs.shape)
        outputs = decoder([tensor1, tensor2])
